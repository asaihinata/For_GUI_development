...
class Funne(twoDElement):
 ...
 def _funne(self,data:np.ndarray,height=1,alpha=1):
  ...
  bars=self.ax.barh(np.arange(len(data)),data,left=(data_max-data)/2,height=height,alpha=alpha)
  for i in bars:i.set_alpha(alpha)
  return bars
 def update(self,data=None,**kw):
  self._updates(**kw)
  if isinstance(data,NpArraytype):self.data=self._dataarr(kw.get('data'))
