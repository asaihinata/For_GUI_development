...
class Manylist(Datalist):
 def __init__(self,data:tuple|list|ndarray=None)->None: '''一次元配列を含む多次元配列を許す配列を作成する。

 :param data: 配列を指定する。
 :type data: tuple|list|ndarray
 :raises TypeError: `data`に配列以外の型を指定した場合に発生させる。'''
 def __iter__(self)->Iterator[list[Any]]:...
 def __len__(self)->int:...
class Onelist(Datalist):
 def __init__(self,data:tuple|list|ndarray=None)->None:'''一次元配列のみを許す配列を作成する。
...
class Conectlist(Datalist):
 def __init__(self,data:tuple|list|ndarray=None)->None:'''配列から一次元配列を作成する。
