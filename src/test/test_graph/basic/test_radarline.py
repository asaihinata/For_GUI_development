...
if __name__=="__main__":
 ...
 layout=[
  ...
  [
   ...
   sgg.RadarLine(data=radarplotdata,marker="+",markersize=20,title="マーカーの大きさを変える")
  ],
  [
   sgg.RadarLine(data=radarplotdata,title="グラフを更新する",key="radarline"),
   sgg.Buttons(text="更新ボタン",function=updates)
  ]
 ]
 win=sgg.window(title="折れ線レーダーチャート(test)",layout=layout,scroll=True,maxmine=True)
 win.run()