...
class Listboxs(_Element):
 ...
 def select_set(self,val:int):'''読み込み時にListboxsウィジェットで選択される項目を指定する。

 :param val: 読み込み時にListboxsウィジェットで選択される項目を指定する。
 :type val: int'''
 def apend(self,place:int|Literal['end']='end',lists:list=[]):'''Listboxsウィジェットに項目を追加する。

 :param lists: Listboxsウィジェットに追加する項目を指定する。
 :type lists: list
 :param place: 追加する場所を指定する。
 :type place: int|Literal['end']'''
 def clear(self):'''Listboxsウィジェットの項目を全て削除する。'''
 def dele(self,*index:int):'''Listboxsウィジェットの指定された箇所の項目を削除する。
 ...
 :rtype: list[Any]|Any'''
 def set(self,lists:tuple[str,...]):'''Listboxsウィジェットの項目をlitsに置き換える。
