...
def range_zero_one(val,out=1.0,endpoint=True):
 if not isinstance(val,np.number):return out
...
def num1s(val=0,mins=1):
 if isinstance(val,np.number) and 1<=val:return val
 return mins
def num0s(val=0,mins=0):
 if isinstance(val,np.number)and 0<=val:return val
 return mins
def num0(val=0,mins=0):
 if isinstance(val,np.number)and 0<val:return val
 return mins
def list2number(lin=None):
 if isinstance(lin,list|tuple) and len(lin)==2 and all(isinstance(i,np.number)for i in lin):return True
 return False
