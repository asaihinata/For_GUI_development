...
class SSBarGraph(twoElement):
 def __init__(self,master,kw):
  ...
  self.plot(self.__data,self.dataname,label=self.label,alpha=self.alpha,width=self.width)
 ...
 def update(self,data=None,dataname=None,**kw):
  ...
  self.plot(self.__data,self.dataname,label=self.label,alpha=self.alpha,width=self.width,align=self.align)
  self._redraw()
