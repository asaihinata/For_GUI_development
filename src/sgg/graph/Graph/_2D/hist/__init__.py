...
bins in ['auto','fd','doane','scott','stone','rice','sturges','sqrt']
  ...
  elif isinstance(bins,int):self.bins=num1s(bins,round(self.__data.sturgesval))
  else:self.bins=round(self.__data.sturgesval)
  ...
  self.plot(self.__data,label=self.label,bins=self.bins,ranges=(self.min,self.max),bottom=self.bottom,orientation=self.orientation,width=self.width,alpha=self.alpha)
 ...
 def update(self,data=None,**kw):
  ...
  self.plot(self.__data,label=self.label,bins=self.bins,ranges=(self.min,self.max),bottom=self.bottom,orientation=self.orientation,width=self.width)
  self._redraw()
