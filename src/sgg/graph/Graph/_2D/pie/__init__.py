...
class Pie(twoElement):
 def __init__(self,master,kw):
  ...
  elif isinstance(explode,int|float):self.explode=[float(explode) for _ in range(self.__data.size)]
  else:self.explode=None
  self.plot(self.__data,startangle=self.startangle,shadow=self.shadow,counterclock=self.counterclock,label=self.label,labeldistance=self.labeldistance,explode=self.explode,startangletype=self.startangletype,alpha=self.alpha)
 ...
 def update(self,data=None,**kw):
  ...
  self.plot(self.__data,startangle=self.startangle,shadow=self.shadow,counterclock=self.counterclock,label=self.label,labeldistance=self.labeldistance,explode=self.explode,startangletype=self.startangletype)
  self._redraw()
