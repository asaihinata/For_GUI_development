...
class Barpolar(polarElement):
 def __init__(self,master,kw):
  ...
  self.plot(self.__x.tonp(),self.__y.tonp(),alpha=self.alpha,width=self.width,align=self.align,color=self.color)
 def plot(self,x,y,alpha=1,width=1,align='center',color=None):
  self.clear()
  self.graphdata=[self.ax.bar(x,y,bottom=0,color=color,alpha=alpha,width=width,align=align)]
  self._adjustment()
 def update(self,x=None,y=None,data=None,**kw):
  ...
  if not isinstance(data,nListlike):data=None
