...
dtype:_DT64Codes
 ...
 __rsub__=__sub__
 @classmethod
 def today(cls,unit:DateUnit='D')->NPDate:'''現在日付(UTC時刻)を返す'''
 @classmethod
 def now(cls,unit:NativeTimeUnit='h')->NPDate:'''現在時刻(UTC時刻)を返す'''
 def tostr(
self,
...
casting:Literal['no','equiv','safe','same_kind','same_value','unsafe']='same_kind'
 ...
 def weekday(self):'''その日付日時の曜日を求める'''
 @overload
 def diff_today(
self,
days:bool=...
...
days:bool=False
 ...
 :param days: 今日を含めるか指定する。
 :type days: bool'''
 def astype(self,dtype:DTypeLike='datetime64[D]')->NPDate:'''`dtype`で指定された型に変更します。'''
 def sort(self)->NPDate:'''`data`にソートを実行する。'''
 def first_pop(self)->NPDate:'''配列の最初の要素のコピーをその配列の末尾に追加する。'''
 def clear(self)->NPDate:'''配列をクリアする'''
 def deep_add(self,val:int)->NPDate:'''配列の深さを`val`分だけ追加する。'''
 def deep_add(self,val:int)->NPDate:'''配列の深さが`val`より低い場合,深さを`val`にする。'''
 def reshape(self,size:tuple[int,...])->NPDate:'''配列の形状を`size`で指定する'''
 def flatten(self)->NPDate:'''配列を一次元に平坦にする'''