...
def chagehex6(val):
 val=val[0][1:]
 return fromiter((int(val[i:i+2],16)for i in range(0,len(val),2)),dtype=uint8)
def chage(val):return array(val[0],dtype=uint8)
def check(name):
 if name[0]=='#':
  ...
  if HEX3_RE.match(name):
   ...
   return fromiter((int(sets(val[i:i+1]),16)for i in range(0,len(val))),dtype=uint8)
